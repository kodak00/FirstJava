...
int main()
{
  ...
  if (err > 0) {
    rdr_stn_err_img(err);
    if (err == 3 && var.cdf == 0) RSL_free_radar(radar);
    return 0;
  }
  ...
  // 6. UF파일 닫기
  if (var.cdf == 0) RSL_free_radar(radar);
...
int rdr_stn_img()
{
  ...
  // 3. 격자자료 그리기
  if (strcmp(var.rdr,"HSR") == 0 || strcmp(var.rdr,"VER") == 0 || strstr(var.rdr,"LNG"))
    rdr_stn_ppi_disp(im, color_lvl, data_lvl);
  else {
    if (strcmp(var.cpi,"CPP") == 0)
      rdr_stn_cappi_disp(im, color_lvl, data_lvl);
    else
      rdr_stn_ppi_disp(im, color_lvl, data_lvl);
  }
 ...
 *  고도별 CAPPI 표출
 *=============================================================================*/
int rdr_stn_cappi_disp(gdImagePtr im, int color_lvl[], float data_lvl[])
{
  short **rays, *ray1;
  float lvl[256], dbz1, rain1;
  float blanks = 131000;
  float az1, s, rate, ht1, ht2;
  float x1, y1, x, y, dd, dd1, dd2, dd_min;
  float zm = 1.0, xo = 0.0, yo = 0.0;
  float nx = 2.0*var.range*0.001, ny = 2.0*var.range*0.001, zx, zy;
  int   max_gate;
  int   swpn, hci_ok, dn, ok;
  int   i, j, k, i1, j1, ia, ja, color1;
  ...
  // 1. PPI 자료 읽기
  if (var.cdf) {
    // 최대 gate 수 확인, 방위각은 0.5도 간격
    max_gate = 0;
    for (k = 0; k < rdr.num_swp; k++) {
      if (rdr.swp[k].num_gate > max_gate) max_gate = rdr.swp[k].num_gate;
    }
    max_angn = 720;

    // CAPPI 저장할 배열 선언
    rays = (short **)malloc(max_angn * sizeof(short *));
    for (j = 0; j < max_angn; j++) {
      rays[j] = (short *)malloc(max_gate * sizeof(short));
    }
    ray1 = (short *)malloc(max_gate * sizeof(short));

    // 방위각 순서로 처리
    status = nc_open(var.fname, 0, &ncid);  // NC파일 open
    for (j = 0; j < max_angn; j++) {
      az1 = 0.25 + j*0.5;

      // Sweep순서로 처리
      for (k = 0; k < rdr.num_swp; k++) {
        // 해당 방위각에 가장 가까운 ray를 찾음
        for (dd_min = 1000, i1 = -1, i = 0; i < rdr.swp[j].num_ray; i++) {
          dd = fabs(az1 - rdr.swp[j].ray[i].az);
          if (dd < dd_min) {
            dd_min = dd;
            i1 = i;
            break;
        ...
        ray1[0] = rdr.swp[j].ray[i1].ray1;
        num_gate[0] = rdr.swp[j].ray[i1].num_gate;
        status = nc_get_vara_short(ncid, rdr.var[var.voln].var_id, ray1, num_gate, rays[j]);

      }
    }

    elev = rdr.swp[var.swpn].elev;
    num_ray = rdr.swp[var.swpn].num_ray;

    rays = (short **)malloc(num_ray * sizeof(short *));
    for (j = 0; j < num_ray; j++) {
      rays[j] = (short *)malloc(rdr.swp[var.swpn].ray[j].num_gate * sizeof(short));
      ray1[0] = rdr.swp[var.swpn].ray[j].ray1;
      num_gate[0] = rdr.swp[var.swpn].ray[j].num_gate;
      status = nc_get_vara_short(ncid, rdr.var[var.voln].var_id, ray1, num_gate, rays[j]);
    }
    status = nc_close(ncid);
  }
  else {
    if ((sweep = radar->v[var.voln]->sweep[var.swpn]) == NULL) return -1;
    elev = sweep->h.elev;
    num_ray = sweep->h.nrays;

    rayf = (float **)malloc(num_ray * sizeof(float *));
    for (j = 0; j < num_ray; j++) {
      ray = sweep->ray[j];
      rayf[j] = (float *)malloc(ray->h.nbins * sizeof(float));
      for (i = 0; i < ray->h.nbins; i++) {
        rayf[j][i] = ray->h.f(ray->range[i]);
      }
    }
  }
  ...
  for (i = 1; i < var.NI; i++) {
    ...
    // 3.4. SWEEP순으로 읽으면서 CAPPI 계산
    ht1 = dd1 = dd = -99;
    for (swpn = 0; swpn < radar->v[var.voln]->h.nsweeps; swpn++) {
      if ((sweep = radar->v[var.voln]->sweep[swpn]) == NULL) continue;

      // 3.4.1. 해당 방위각의 ray 위치 확인
      ja = swp_ray[swpn][j1];
      if (ja < 0) ja += sweep->h.nrays;
      if (ja >= sweep->h.nrays) ja -= sweep->h.nrays;

      // 3.4.2. 해당 bin의 위치 확인
      i1 = (int)(s*4.0/gate_size[swpn]);
      if (i1 < 0 || i1 >= nbin_max[swpn]*4-1) break;
      ht2 = swp_h[swpn][i1];    // 해당 위치의 고도(m)
      ia = swp_bin[swpn][i1];
      if (ia < 0 || ia >= nbin_max[swpn]) break;

      // 3.4.3. 해당위치의 자료 읽기
      ray = sweep->ray[ja];
      dd2 = ray->h.f(ray->range[ia]);
      ...
      else if ((ht1 > 0 && ht1 <= var.cappi_ht) && ht2 >= var.cappi_ht) {
        if (dd1 > -90 && dd1 < blanks) {
          if (dd2 > -90 && dd2 < blanks) {
            if (hci_ok) {
              if (fabs(var.cappi_ht-ht1) <= fabs(ht2-var.cappi_ht))
                dd = dd1;
              else
                dd = dd2;
            }
            else {
              dd = dd1*(ht2 - var.cappi_ht)/(ht2 - ht1) + dd2*(var.cappi_ht - ht1)/(ht2 - ht1);
            }
          }
          else
            dd = dd1;
 ...
 *  SWEEP별 PPI 표출
 *=============================================================================*/
int rdr_stn_ppi_disp(gdImagePtr im, int color_lvl[], float data_lvl[])
{
  int   ncid;   // NetCDF ID
  short **rays;
  float **rayf;
  float lvl[256], dbz1, rain1;
  float blank1s = BLANK1*0.01, blank3s = BLANK3*0.01;
  float az1, s, rate, ht1, elev;
  float x1, y1, x, y, dd;
  float zm = 1.0, xo = 0.0, yo = 0.0;
  float nx = 2.0*var.range*0.001, ny = 2.0*var.range*0.001, zx, zy;
  int   num_ray;    // PPI층내 ray수
  int   hci_ok = 0, dn, code, ok;
  int   status;
  int   i, j, k, i1, j1, ia, ja, color1;
  size_t  ray1[] = {0};
  size_t  num_gate[] = {0};
  ...
  if (var.cdf) {
    elev = rdr.swp[var.swpn].elev;
    num_ray = rdr.swp[var.swpn].num_ray;
    status = nc_open(var.fname, 0, &ncid);
  ...
  for (i = 1; i < var.NI; i++) {
    ...
    s = sqrt(x*x + y*y);
    i1 = (int)(s*4.0/gate_size[var.swpn]);
    if (i1 < 0 || i1 >= nbin_max[var.swpn]*4-1) continue;
    ht1 = swp_h[var.swpn][i1];    // 해당 위치의 고도(m)
    ia = swp_bin[var.swpn][i1];
    if (ia < 0 || ia >= nbin_max[var.swpn]) continue;

    // 4.4. 해당위치의 자료 읽기
    if (ja < 0) ja += num_ray;
    if (ja >= num_ray) ja -= num_ray;

    if (var.cdf) {
      dd = (float)rays[ja][ia];
      if (dd > rdr.var[var.voln].blank) dd *= rdr.var[var.voln].scale;
    }
    else {
      //ray = sweep->ray[ja];
      //dd = ray->h.f(ray->range[ia]);
      dd = rayf[ja][ia];
    }
    if (dd > 131000 || dd < -32760) dd = blank3s;
...
int ring_disp(gdImagePtr im, int color_lvl[])
{
  ...
  if (strcmp(var.rdr,"VER") == 0) {
    gdImageLine(im, 0, var.NJ/2+TITLE_pixel, var.NI,  var.NJ/2+TITLE_pixel, color_lvl[249]);
    gdImageLine(im, var.NI/2, TITLE_pixel, var.NI/2,  var.GJ, color_lvl[249]);
    gdImageLine(im, 0, TITLE_pixel, var.NI, var.GJ, color_lvl[249]);
    gdImageLine(im, 0, var.GJ, var.NI, TITLE_pixel, color_lvl[249]);
  }
  ...
  // 2. 동심원 표시(50km 간격)
    ...
    for (k = 50; k <= var.range*0.001; k += 50) {
      rg = 2*(float)k/rate;
      gdImageArc(im, var.NI/2, var.NJ/2+TITLE_pixel, (int)rg, (int)rg, 0, 360, color_lvl[249]);
    }
  }

  // 3. RHI인 경우, 선택된 각도를 표시
  if (var.area == 9) {
...
int title_disp(gdImagePtr im, int color_lvl[])
{
  ...
  gdImageString(im, gdFontLarge, x, TITLE_pixel+3, text, color_lvl[244]);
  gdImageString(im, gdFontLarge, x+1, TITLE_pixel+3, text, color_lvl[244]);

  // 5. pulse width
  sprintf(text, "%.0fus", var.pulse_width);
  x = var.NI - strlen(text)*8.5 - 10;
  gdImageString(im, gdFontLarge, x, TITLE_pixel+18, text, color_lvl[244]);
  gdImageString(im, gdFontLarge, x+1, TITLE_pixel+18, text, color_lvl[244]);
