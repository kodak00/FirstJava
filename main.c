...
static int fnDataDispCompMultiwind(MULTIWIND **stWind)
{
    ...
    int         nFreq           = 0;
    //oskim 20190304
    float       fLon            = 122.449535;
    float       fLat            = 32.213078;
    //float       fLon            = 122.150940;  //좌하단 경도
    //float       fLat            = 30.846172;   //좌하단 위도
    float       fX_Point        = 0.0;
    float       fY_Point        = 0.0;
    float       fImgXGridM      = 0.0;
    float       fImgYGridM      = 0.0;
    //oskim 20190304
    float       fWindGrid_Km    = 1.0;
    //float       fWindGrid_Km    = 2.0;
    int         nDiff_X         = 0;
    int         nDiff_Y         = 0;
    int         nResult_X       = 0;
    int         nResult_Y       = 0;
        ...
        for(nXIdx = 0; nXIdx < GRID_X_CNT; nXIdx+=nFreq)
        {
            if (stWind[nYIdx][nXIdx].u != BAD_VALUE_F && stWind[nYIdx][nXIdx].v != BAD_VALUE_F)
            {
                ...
                if(g_option.m_cUnitFlag == 'W')
                {
                    ...
                    fnWind_Draw(pImg, nResult_X, nResult_Y, ws, wd, nWindColor, nP_Size, nWingSize);
                }
...
*/
    ...
    for(nZIdx = 0; nZIdx < nZdim; nZIdx++)
    {
        for (nYIdx = 0; nYIdx < nYdim; nYIdx++)
        {
            for (nXIdx = 0; nXIdx < nXdim; nXIdx++)
            {
                if(nZIdx != nAltitude) continue;
                //if(nZIdx != 0) continue;

                lOneDimIdx = (nZIdx*nYdim + nYIdx)*nXdim + nXIdx;

                fU_Val = (pU[lOneDimIdx] - hdr.data_minus) / hdr.data_scale;
                fV_Val = (pV[lOneDimIdx] - hdr.data_minus) / hdr.data_scale;
                fW_Val = (pW[lOneDimIdx] - hdr.data_minus) / hdr.data_scale;

                if (pU[lOneDimIdx] == -30000 || pV[lOneDimIdx] == -30000 ||  pW[lOneDimIdx] == -30000 || fU_Val < -990.0f || fV_Val < -990.0f)
                {
                    wind[nYIdx][nXIdx].u = BAD_VALUE_F;
                    wind[nYIdx][nXIdx].v = BAD_VALUE_F;
                }
